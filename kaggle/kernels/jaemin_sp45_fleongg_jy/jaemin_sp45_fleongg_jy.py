...
def main():
    ...
    return sub, cv_final, test_df, meta_test, test_pred

sub, cv_final, _fleongg_test_df, _fleongg_meta_test, _fleongg_test_pred = main()
sub.head()
...
# %% cell: optional JY dynamic inference on the fleongg feature frame
def _find_jy_model_dir():
    import glob as _jy_glob
    from pathlib import Path as _JyPath
    for _cfg in _jy_glob.glob("/kaggle/input/**/ensemble_config.pkl", recursive=True):
        _d = _JyPath(_cfg).parent
        if (_d / "models" / "lgb0.pkl").exists() and (_d / "models" / "lgb1.pkl").exists() and (_d / "models" / "lgb2.pkl").exists():
            return _d
    return None


def _jy_predict_from_features(_test_df):
    import joblib as _jy_joblib
    import numpy as _jy_np
    import pandas as _jy_pd

    _jy_dir = _find_jy_model_dir()
    if _jy_dir is None:
        print("JY model package not found; skipping JY dynamic inference", flush=True)
        return None, None

    _cfg = _jy_joblib.load(_jy_dir / "ensemble_config.pkl")
    _features = list(_cfg.get("feature_cols", []))
    if not _features:
        raise RuntimeError("JY ensemble_config.pkl does not contain feature_cols")

    _frame = _test_df.copy()
    _missing = [c for c in _features if c not in _frame.columns]
    for _c in _missing:
        _frame[_c] = 0.0
    _x = _frame[_features].replace([_jy_np.inf, -_jy_np.inf], _jy_np.nan).fillna(0.0)

    _raw = _jy_np.zeros(len(_frame), dtype=_jy_np.float64)
    _model_rows = []
    for _name, _weight in dict(_cfg.get("weights", {})).items():
        _models = _jy_joblib.load(_jy_dir / "models" / f"{_name}.pkl")
        _pred = _jy_np.mean([_m.predict(_x) for _m in _models], axis=0)
        _raw += float(_weight) * _pred
        _model_rows.append({
            "model": _name,
            "weight": float(_weight),
            "pred_mean": float(_jy_np.mean(_pred)),
            "pred_std": float(_jy_np.std(_pred)),
        })

    _last = _frame["last_known_tvt"].to_numpy(dtype=_jy_np.float64)
    _pf_delta = _frame["pf_ancc"].to_numpy(dtype=_jy_np.float64) - _last
    _md_since = _frame["md_since"].to_numpy(dtype=_jy_np.float64)
    _alpha = float(_cfg.get("alpha", 1.0))
    _tau = float(_cfg.get("tau", 50.0))
    _w_pf = float(_cfg.get("w_pf", 0.15))
    _warm = 1.0 - _jy_np.exp(-_jy_np.maximum(_md_since, 0.0) / max(_tau, 1e-6))
    _delta = _alpha * _warm * ((1.0 - _w_pf) * _raw + _w_pf * _pf_delta)
    _pred_tvt = _last + _delta

    _sub = _jy_pd.DataFrame({"id": _frame["id"].to_numpy(), "tvt": _pred_tvt})
    _summary = {
        "jy_dir": str(_jy_dir),
        "rows": int(len(_sub)),
        "missing_feature_count": int(len(_missing)),
        "missing_features_preview": _missing[:25],
        "alpha": _alpha,
        "tau": _tau,
        "w_pf": _w_pf,
        "raw_delta_mean": float(_jy_np.mean(_raw)),
        "raw_delta_std": float(_jy_np.std(_raw)),
        "pred_mean": float(_jy_np.mean(_pred_tvt)),
        "pred_std": float(_jy_np.std(_pred_tvt)),
        "models": _model_rows,
    }
    return _sub, _summary


_jy_dynamic_submission, _jy_dynamic_summary = _jy_predict_from_features(_fleongg_test_df)


# %% cell: final dynamic blend of sp45 projection, fleongg pretrained inference, and optional JY dynamic inference
...
if _jy_dynamic_submission is not None:
    _jy_dynamic_submission.to_csv(_WORK / 'jy_dynamic_submission.csv', index=False)
    (_WORK / 'jy_dynamic_summary.json').write_text(_final_json.dumps(_jy_dynamic_summary, indent=2), encoding='utf-8')
...
if _jy_dynamic_submission is not None:
    _merged = _merged.merge(
        _jy_dynamic_submission[['id', 'tvt']].rename(columns={'tvt': 'tvt_jy'}),
        on='id',
        how='inner',
    )
...
for _col in [c for c in ['tvt_sp45', 'tvt_fleongg', 'tvt_jy'] if c in _merged.columns]:
...
if 'tvt_jy' in _merged.columns:
    for _w_sp45, _w_fle, _w_jy, _tag in [
        (0.50, 0.40, 0.10, 'sp50_fle40_jy10'),
        (0.48, 0.40, 0.12, 'sp48_fle40_jy12'),
        (0.45, 0.40, 0.15, 'sp45_fle40_jy15'),
        (0.50, 0.35, 0.15, 'sp50_fle35_jy15'),
        (0.55, 0.35, 0.10, 'sp55_fle35_jy10'),
    ]:
        _out = _merged[['id']].copy()
        _out['tvt'] = (
            _w_sp45 * _merged['tvt_sp45'].astype(float)
            + _w_fle * _merged['tvt_fleongg'].astype(float)
            + _w_jy * _merged['tvt_jy'].astype(float)
        )
        _name = f'submission_{_tag}.csv'
        _out.to_csv(_WORK / _name, index=False)
        _diff = _out['tvt'].to_numpy(dtype=float) - _merged['tvt_sp45'].to_numpy(dtype=float)
        _rows.append({
            'file': _name,
...
_base = _merged[['id']].copy()
_base['tvt'] = 0.55 * _merged['tvt_sp45'].astype(float) + 0.45 * _merged['tvt_fleongg'].astype(float)
if 'tvt_jy' in _merged.columns:
    _sp = _merged['tvt_sp45'].to_numpy(dtype=float)
    _fle = _merged['tvt_fleongg'].to_numpy(dtype=float)
    _jy = _merged['tvt_jy'].to_numpy(dtype=float)
    _base_v = _base['tvt'].to_numpy(dtype=float)
    _disagree = _final_np.abs(_sp - _fle)
    _jy_dist = _final_np.abs(_jy - _base_v)
    _uncertainty_gate = _final_np.clip((_disagree - 6.0) / 24.0, 0.0, 1.0)
    _agreement_gate = _final_np.exp(-(_jy_dist / 28.0) ** 2)
    _w_jy = _final_np.clip(0.06 + 0.08 * _uncertainty_gate * _agreement_gate, 0.04, 0.14)
    _out = _merged[['id']].copy()
    _out['tvt'] = (1.0 - _w_jy) * _base_v + _w_jy * _jy
    _name = 'submission_consensus_gated_jy.csv'
    _out.to_csv(_WORK / _name, index=False)
    _rows.append({
        'file': _name,
        'w_sp45': '0.55_base',
        'w_fleongg': '0.45_base',
        'w_jy_mean': float(_final_np.mean(_w_jy)),
        'w_jy_min': float(_final_np.min(_w_jy)),
        'w_jy_max': float(_final_np.max(_w_jy)),
        'rows': len(_out),
        'mean_tvt': float(_out['tvt'].mean()),
        'std_tvt': float(_out['tvt'].std()),
        'rmse_vs_sp45': float(_final_np.sqrt(_final_np.mean((_out['tvt'].to_numpy(dtype=float) - _sp) ** 2))),
        'p95_abs_vs_sp45': float(_final_np.quantile(_final_np.abs(_out['tvt'].to_numpy(dtype=float) - _sp), 0.95)),
    })
    _final_name = _name
else:
