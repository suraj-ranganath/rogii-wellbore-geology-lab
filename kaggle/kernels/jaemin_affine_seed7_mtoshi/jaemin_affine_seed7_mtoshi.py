...
try:
    ...
    _W_ANCHOR = -0.24
    if abs(_W_ANCHOR) > 1e-12:
        _anchor_bl = _blend_pd.read_csv(_blend_w / 'submission_sp45_fleongg_w0.55.csv')
        _merged_bl = _merged_bl.merge(
            _anchor_bl[["id", "tvt"]].rename(columns={"tvt": "tvt_anchor"}),
            on="id",
            how="inner",
        )
    ...
    for _col in ["tvt_seed7", "tvt_mtoshi", "tvt_beicicc", "tvt_anchor"]:
        if _col not in _merged_bl.columns:
            continue
    ...
    _final_bl["tvt"] = (
        ...
        + _W_ANCHOR * _merged_bl.get("tvt_anchor", 0.0)
    )
    ...
    _summary_bl = {
        ...
        "anchor_mean": float(_merged_bl["tvt_anchor"].mean()) if "tvt_anchor" in _merged_bl.columns else None,
