...
| Target-free alignment gated | `uv run kaggle kernels push -p kaggle/kernels/target_free_alignment_gated` | Kaggle GPU kernel | queued locally | Private script-kernel copy of public `pilkwang/rogii-eda-target-free-alignment-for-tvt`, using `full_stack_sel15_gated_model_gated` with same-well physical shortcut disabled. Kaggle currently has both allowed batch GPU sessions occupied, so `scripts/queue_target_free_push.py` waits for a slot and pushes a fresh `surajranganath17/rogii-strat-align-sidecar-vN` slug, recording the accepted slug in `outputs/target_free_push_state.json`. Monitor will submit only after completion, CSV validation, and available daily slot. |
