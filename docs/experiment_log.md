...
| Sunny+v10 artifact blend | `uv run kaggle competitions submit rogii-wellbore-geology-prediction -k surajranganath17/rogii-sunny-v10-artifact-blend -v 1 -f submission.csv -m "sunny v10 artifact blend"` | Kaggle code submission | pending | Submission ref `53356985`; reconstructs the public `kojimar/rogii-physical-pf-signal-meets-artifact-stack` 0.80 Sunny physical/PF + 0.20 v10 artifact-stack blend without using its inaccessible helper dataset; output validated locally before monitor submitted. |
| Target-free alignment gated | `uv run kaggle competitions submit rogii-wellbore-geology-prediction -k surajranganath17/rogii-strat-align-sidecar-v2 -v 1 -f submission.csv -m "target free alignment gated"` | Kaggle code submission | pending | Submission ref `53359947`; private script-kernel copy of public `pilkwang/rogii-eda-target-free-alignment-for-tvt`, using `full_stack_sel15_gated_model_gated` with same-well physical shortcut disabled. Accepted slug `surajranganath17/rogii-strat-align-sidecar-v2`; output validated locally before monitor submitted. |
