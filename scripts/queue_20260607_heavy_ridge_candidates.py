...
STATE_PATH = Path("outputs/queue_20260607_heavy_ridge_state.json")
...
CANDIDATES = [
    Candidate(
        name="ridge_w040_ridge042",
        kernel="surajranganath17/rogii-ridge-w040-ridge042",
        version=1,
        message="ridge w040 heavy final blend 042",
        kernel_dir=Path("kaggle/kernels/ridge_w040_ridge042"),
        output_dir=Path("outputs/queue_20260607_heavy_ridge/ridge_w040_ridge042"),
    ),
    Candidate(
        name="ridge_w040_ridge045",
        kernel="surajranganath17/rogii-ridge-w040-ridge045",
        version=1,
        message="ridge w040 heavy final blend 045",
        kernel_dir=Path("kaggle/kernels/ridge_w040_ridge045"),
        output_dir=Path("outputs/queue_20260607_heavy_ridge/ridge_w040_ridge045"),
    ),
    Candidate(
        name="ridge_w040_ridge050",
        kernel="surajranganath17/rogii-ridge-w040-ridge050",
        version=1,
        message="ridge w040 heavy final blend 050",
        kernel_dir=Path("kaggle/kernels/ridge_w040_ridge050"),
        output_dir=Path("outputs/queue_20260607_heavy_ridge/ridge_w040_ridge050"),
    ),
    Candidate(
        name="ridge_w040_ridge060",
        kernel="surajranganath17/rogii-ridge-w040-ridge060",
        version=1,
        message="ridge w040 heavy final blend 060",
        kernel_dir=Path("kaggle/kernels/ridge_w040_ridge060"),
        output_dir=Path("outputs/queue_20260607_heavy_ridge/ridge_w040_ridge060"),
    ),
    Candidate(
        name="ridge_w040_ridge070",
        kernel="surajranganath17/rogii-ridge-w040-ridge070",
        version=1,
        message="ridge w040 heavy final blend 070",
        kernel_dir=Path("kaggle/kernels/ridge_w040_ridge070"),
        output_dir=Path("outputs/queue_20260607_heavy_ridge/ridge_w040_ridge070"),
    ),
]
...
def push_candidate(candidate: Candidate, state: dict) -> None:
    ...
    push_output = f"{result.stdout}\n{result.stderr}"
    if result.returncode == 0 and "successfully pushed" in push_output:
...
def output_is_ready(candidate: Candidate) -> Path | None:
    try:
        output_path = download_output(candidate)
        validate_submission(output_path)
    except Exception as exc:
        print(f"{candidate.name}: output not ready or invalid: {exc}", flush=True)
        return None
    return output_path
...
def monitor(timeout_minutes: int, poll_seconds: int) -> None:
    ...
    while time.time() - start < timeout_minutes * 60:
        ...
        for candidate in CANDIDATES:
            ...
            if "COMPLETE" in status or "UNAVAILABLE" in status or "UNKNOWN" in status:
                output_path = output_is_ready(candidate)
